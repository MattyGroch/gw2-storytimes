...
const EXCLUDED_QUEST_IDS = new Set([587, 600, 602, 606]);
...
const SEASON_GROUPS = {
  ...
  '5F35F25C-AE33-4D92-A061-227CE54FA5DC': [
    { label: 'Act 1', count: 5 }, { label: 'Act 2', count: 4 }, { label: 'Act 3', count: 5 },
  ],
};
...
async function fetchMissionsFromAPI(existingSeedData = {}) {
  ...
  console.log(`  Fetched ${Object.keys(questsById).length} quests`);
  ...
  for (const apiSeason of knownSeasons) {
    ...
    seasons.push({ id: apiSeason.id, name: apiSeason.name, order: orderIndex + 1 });
    ...
    for (const story of seasonStories) {
      ...
      if (races && races.length === 1) storyName = `${story.name} (${races[0]})`;
  ...
  for (const storyMissions of Object.values(byStory)) {
    const nameCounts = {};
    for (const m of storyMissions) nameCounts[m.name] = (nameCounts[m.name] || 0) + 1;
    for (const [name, count] of Object.entries(nameCounts)) {
      if (count === 2) {
        let part = 0;
        for (const m of storyMissions) {
          if (m.name === name) { part++; m.name = `${name} (Part ${part})`; dupeCount++; }
        }
      } else if (count >= 3) {
        const group = storyMissions.filter(m => m.name === name).sort((a, b) => a.id - b.id);
        const canonicalId = group[0].id;
        for (const m of group) {
          if (m.id !== canonicalId) { m.canonical_id = canonicalId; canonicalCount++; }
        }
      }
    }
  }
  ...
  for (const m of existingSeedData.missions || []) {
  ...
  if (Object.keys(existingMissionData).length > 0) {
  ...
  return { seasons, stories, missions };
}

module.exports = { fetchMissionsFromAPI, SEASON_ORDER, SEASON_GROUPS };
